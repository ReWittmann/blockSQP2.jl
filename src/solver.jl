function SQPstats(outpath::String)
    ...
    return ccall(@dlsym(libblockSQP, "create_SQPstats"), Ptr{Cvoid}, (Ptr{Cchar},), pointer(reinterpret(Cchar, Ctrans)))
end
...
mutable struct Solver
    ...
    Solver(J_prob::blockSQPProblem, opts::blockSQPOptions, SQPstats_obj_pass::Ptr{Cvoid}) = begin        
        ...
        ccall(@dlsym(libblockSQP, "Problemspec_set_dense_eval"), Cvoid,
            (Ptr{Cvoid}, Ptr{Cvoid}),
            new_Problemspec_obj,
        ...
        ccall(@dlsym(libblockSQP, "Problemspec_set_blockIdx"), Cvoid,
            (Ptr{Cvoid}, Ptr{Cint}, Cint), new_Problemspec_obj, pointer(J_prob.blockIdx), Cint(length(J_prob.blockIdx) - 1))
        
        ccall(@dlsym(libblockSQP, "Problemspec_set_nnz"), Cvoid,
            (Ptr{Cvoid}, Cint), new_Problemspec_obj, Cint(J_prob.nnz))
        
        ccall(@dlsym(libblockSQP, "Problemspec_set_bounds"), Cvoid,
            (Ptr{Cvoid}, Ptr{Cdouble}, Ptr{Cdouble},
        ...
        new_SQPmethod_obj = ccall(@dlsym(libblockSQP, "create_SQPmethod"), Ptr{Cvoid}, (Ptr{Cvoid}, Ptr{Cvoid}, Ptr{Cvoid}), new_Problemspec_obj, new_SQPoptions_obj, SQPstats_obj_pass)
        if new_SQPmethod_obj == C_NULL
            error(unsafe_string(ccall(@dlsym(libblockSQP, "get_error_message"), Ptr{Cchar}, ())))
        end
        ...
        function Solver_finalizer(arg_sol::Solver)
            ccall(@dlsym(libblockSQP, "delete_SQPmethod"), Cvoid, (Ptr{Cvoid},), arg_sol.SQPmethod_obj)
            ccall(@dlsym(libblockSQP, "delete_Problemspec"), Cvoid, (Ptr{Cvoid},), arg_sol.Problemspec_obj)
            ccall(@dlsym(libblockSQP, "delete_SQPoptions"), Cvoid, (Ptr{Cvoid},), arg_sol.SQPoptions_obj)
            ccall(@dlsym(libblockSQP, "delete_QPsolver_options"), Cvoid, (Ptr{Cvoid},), arg_sol.QPsolver_options_obj)
            ccall(@dlsym(libblockSQP, "delete_SQPstats"), Cvoid, (Ptr{Cvoid},), arg_sol.SQPstats_obj)
        end
...
function init!(sol::Solver)
    ccall(@dlsym(libblockSQP, "SQPmethod_init"), Cvoid, (Ptr{Cvoid},), sol.SQPmethod_obj)
end

function run!(sol::Solver, maxIt::Integer, warmStart::Integer)
    ret::Cint = ccall(@dlsym(libblockSQP, "SQPmethod_run"), Cint, (Ptr{Cvoid}, Cint, Cint), sol.SQPmethod_obj, Cint(maxIt), Cint(warmStart))
