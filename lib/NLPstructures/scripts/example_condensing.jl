...
matchings = BlockDescriptor{nlpMatching}(tag = :matchings)
cond_target = BlockDescriptor{nlpCondensingTarget}(tag = :cond, matchings = matchings)


h0 = BlockDescriptor{nlpHess}(tag = :h0, parent = cond_target)
h1 = BlockDescriptor{nlpHess}(tag = :h1, parent = cond_target)
h2 = BlockDescriptor{nlpHess}(tag = :h2, parent = cond_target)
h3 = BlockDescriptor{nlpHess}(tag = :h3, parent = cond_target)
...
vLayout = TupleBD[(cond_target, [(h0, [(u0, 1)]), (h1, [(x1, 2), (u1, 1)]), (h2, [(x2, 2), (u2, 1)]), (h3,[(x3, 2), (u3, 1)])])]
cLayout = TupleBD[(matchings, [(m1, 2), (m2, 2), (m3, 2)]), (constr, 1)]

vBlocks = [cond_target, h0, h1, h2, h3, u0, x1, u1, x2, u2, x3, u3]
cBlocks = [matchings, m1, m2, m3, constr]

NLPlayout = NLPstructure((vBlocks...,), to_Axis(vLayout), (cBlocks...,), to_Axis(cLayout))

# NLPlayout2 = NLPstructure((vBlocks...,), vLayout, (cBlocks...,), cLayout)
...
#From blockSQP.jl condensing example
...
#TODO: Allow creating a Condener from 
# cond_obj = blockSQP.Condenser(NLPlayout, dep_bounds = 2)
...
condensed_h, condensed_jacobian, condensed_hess,
    ...
    blockSQP.full_condense!(cond_data, grad_obj, con_jac, hess, lb_var, ub_var, lb_con, ub_con)
