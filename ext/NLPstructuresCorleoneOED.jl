...
function NLPstructures.extract_preLayouts(OED::OEDLayer,
    ...
    return NLPstructures.extract_preLayouts(OED.layer, ps, st; name = name)
end

function NLPstructures.extract_preLayouts(MultiExp::MultiExperimentLayer,
    ...
    # subPreLayouts = (NLPstructures.extract_preLayouts(MultiExp.layers[i], ps[Symbol(:experiment_, i)], st[Symbol(:experiment_, i)]; name = Symbol(name, :_, i))
    #                     for i in enumerate(MultiExp.n_exp))
    
    subPreLayouts = ((NLPstructures.extract_preLayouts(MultiExp.layers[i], ps[Symbol(:experiment_, i)], st[Symbol(:experiment_, i)]; name = Symbol(name, :_, i))
                        for i in Base.OneTo(MultiExp.n_exp))...,)
    # subPreLayouts2 = [NLPstructures.extract_preLayouts(MultiExp.layers[i], ps[Symbol(:experiment_, i)], st[Symbol(:experiment_, i)]; name = Symbol(name, :_, i))
    #                     for i in Base.OneTo(MultiExp.n_exp)]
